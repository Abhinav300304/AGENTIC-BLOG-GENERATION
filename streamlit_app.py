...
# # --- UI Elements ---
# st.title("✍️ Agentic Blog Generator")
# st.markdown("This interface uses an agentic backend to generate high-quality blog posts in multiple languages.")
...
#         st.error("Please enter a topic to generate the blog.")
#     else:
#         # Define the endpoint of your FastAPI backend
#         api_url = "http://127.0.0.1:8000/blogs"
...
#                     # Render the structured blog content
#                     if blog_data:
#                         st.markdown("---")
#                         st.header(blog_data.get("main_title", "Blog Title"))
#                         st.markdown(f"*{blog_data.get('introduction', '')}*")
...
#                             st.markdown(section.get("content", ""))
#                     else:
#                         st.error("Received an empty response from the backend. Please check the backend logs.")

#                 else:
#                     # Display an error message if the request failed
#                     st.error(f"Failed to generate blog. Status code: {response.status_code}")
#                     st.json(response.json()) # Show the detailed error from the backend

#         except requests.exceptions.RequestException as e:
#             st.error(f"Could not connect to the backend at {api_url}.")
#             st.error(f"Please ensure your FastAPI server is running. Error: {e}")